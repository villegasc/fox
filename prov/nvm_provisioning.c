...
struct free_blk* prov_init_fblk(int ch, int lun, int blk){
...
int prov_get_free_from_bbt(const struct nvm_bbt* bbt, 
                                   struct nvm_lun* lun ){
    int blk, pl;
    int idx;
    int ch, l;
    ...
    for (blk=0; blk < nblks; blk+=nplanes){
        ...
        if (!(bad_blk)){
            ...
            lun->nfree_blks++;
            LIST_INSERT_HEAD(&(lun->free_blk_head),
                                lun->index[idx], entry);
            idx++;
        } else {
            lun->index[idx] = NULL;
...
void prov_exit_fblk_lun(struct nvm_lun* lun){
    uint32_t i;
...
int prov_update_fblk_list(struct nvm_lun* lun, uint32_t blk_idx){
    pthread_mutex_lock(&(lun->l_mutex));
...
ssize_t prov_vblock_pwrite_next(struct nvm_vblk* vblk, const void *buf, 
                            size_t count){ 
    return prov_vblock_pwrite(vblk, buf, count, vblk->pos_write);
}
